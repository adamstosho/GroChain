...
export const usePriceAlerts = () => {
  ...
  const fetchAlerts = useCallback(async (page = 1, limit = 10, isActive?: boolean) => {
    try {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to fetch price alerts'
  ...
  const updateAlert = useCallback(async (alertId: string, data: UpdatePriceAlertData) => {
    try {
      ...
      const errorMessage = error.response?.data?.message || error.message || 'Failed to update price alert'
      setError(errorMessage)
